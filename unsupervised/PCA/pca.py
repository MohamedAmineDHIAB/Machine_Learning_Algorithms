...
def pca(X, n):
    ...
    # project the meaned data on these n principal components
...
if __name__ == '__main__':
    ...
    data_styled = data.head().style.background_gradient()
    dfi.export(data_styled, './figs/original_iris.png')
    # prepare the data
    x = data.iloc[:, 0:4]
    ...
    reduced_df = pd.concat([x_reduced, pd.DataFrame(target)], axis=1)

    plt.figure(figsize=(6, 6))
    sb.scatterplot(data=reduced_df, x='Principal Comp 1', y='Principal Comp 2',
                   hue="target", s=60, palette='bright', style="target")
    plt.savefig('./figs/pca_iris.png')
